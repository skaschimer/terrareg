...
// TestProviderSearch tests the provider search functionality.
// This is the Go implementation of Python's test_provider_search.py.
...
func testProviderSearchResultCards(t *testing.T) {
	...
	// Python: self.selenium_instance.get(self.get_url('/search/providers?q=providersearch'))
	...
	for i, expectedHeading := range expectedCardHeadings {
		...
		sourceSelector := "#results .card:nth-child(" + intToStr(i+1) + ") .card-source-link"
		st.AssertTextContent(sourceSelector, expectedSources[i])
	}
}
...
func testProviderSearchFilters(t *testing.T) {
	...
	// Python: self.assert_equals(lambda: len(self.selenium_instance.find_element(By.ID, 'results').find_elements(By.CLASS_NAME, 'card')), 7)
	st.AssertElementVisible("#results .card:nth-child(7)")
...
func testProviderSearchNextPrevButtons(t *testing.T) {
	...
	// Verify 3 results
	st.AssertElementVisible("#results .card:nth-child(3)")
	...
	// Verify 10 results on first page
	st.AssertElementVisible("#results .card:nth-child(10)")
...
func testProviderSearchResultRelevancyOrdering(t *testing.T) {
	...
	// Python: self.selenium_instance.get(self.get_url('/search/providers?q=namematch'))
